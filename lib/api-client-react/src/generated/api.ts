...
export const getGetJobMatchesUrl = (params: GetJobMatchesParams) => {
  ...
  return stringifiedParams.length > 0
    ? `/api/jobs/matches?${stringifiedParams}`
    : `/api/jobs/matches`;
};
...
export const getJobMatches = async (
  ...
  return customFetch<Job[]>(getGetJobMatchesUrl(params), {
    ...options,
    method: "GET",
  });
};
...
export const getGetJobMatchesQueryKey = (params?: GetJobMatchesParams) => {
  return [`/api/jobs/matches`, ...(params ? [params] : [])] as const;
};
...
>(
  ...
  const queryFn: QueryFunction<Awaited<ReturnType<typeof getJobMatches>>> = ({
    signal,
  }) => getJobMatches(params, { signal, ...requestOptions });
